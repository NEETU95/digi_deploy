        ...
        if correspondence_start is None:
            ...
            if author_name.lower() in weekly_doc.text.lower():
                ...
                if index_department_of != -1:
                    ...
                    department_text = text_up_to_doi[index_department_of + len(department) + 1:].strip()
                    print(department_text)

                organization_list = department_text.split(',')

                for org in organization_list:
                    if "university" not in org and org != city_ and org != found_countries:
                        organization = org
                        if organization:
            ...
            # doc = nlp(extracted_text_after_author)
            #
            # for ent in doc.ents:
            #     if ent.label_ == "ORG":
            ...
            modified_text = desired_text.replace(' 1', '')
            cleaned_text = modified_text.replace('', '')
            print("clen text",cleaned_text)
            # address
            ...
            # Phone number
            phone_text=department_text.split('')[0]
            phone_lines=phone_text.split(',')
            ...
            addresses = [token.text for token in doc if (
                    ...
                    and
                    token.text != department_correspondence
            )]
            ...
            # for ent in doc.ents:
            #     if ent.label_ == "ORG":
            #         organization_name_correspondence = ent.text
            ...
            #   report_sent_to_regulatory_authority_by_reports_correspondence)
            report_sent_to_regulatory_authority_by_reports_correspondence ="Unk"
            ...
            # department and organization of correspondence
            department_text_correspondence = ""
            ...
            if index_department_of != -1:
                ...
                department_text_correspondence = correspondence_text[index_department_of + len(department) + 1:].strip()
                print(department_text_correspondence)

            organization_list_corr = department_text_correspondence.split(',')

            for org in organization_list_corr:
                if "university" not in org and org != email_address_correspondence and org != country_correspondence and org != city_correspondence:
                    organization_corr = org

                    if organization_corr:
                        organization_name_correspondence = organization_corr
                        break
            department_text_for_corr = ""
            if department_correspondence=="":
                if author_name.lower() in weekly_doc.text.lower():
                    # Find the index of the specific word
                    word_index = weekly_doc.text.find(author_name)
                    print("13")
                    ...
                    for line in extracted_text_after_author.split("\n"):
                        if "DOI:" in line:
                            break  # Stop when the line containing "DOI:" is found
                        text_up_to_doi += line
                    index_correspondence_department_of = text_up_to_doi.find(' 2')
                    end_index_correspondence_department_of = text_up_to_doi.find(' 3')
                    print("14", text_up_to_doi)
                    print("15", index_correspondence_department_of)
                    if index_correspondence_department_of != -1:
                        print("yes yes yes")
                        #department = text_up_to_doi[index_department_of:].strip().split(",")[0]
                        department_text_for_corr = text_up_to_doi[index_correspondence_department_of + len(' 2'):end_index_correspondence_department_of].strip()

                        print("department text to find out corr", department_text_for_corr)

                    index_correspondence_department_extraction = text_up_to_doi.find('')
                    print("14", text_up_to_doi)
                    print("15", index_correspondence_department_of)
                    # Find 'Department of'
                    index_correspondence_department_extraction = department_text_for_corr.find('Department of')

                    # Find 'Division of'
                    index_correspondence_division_extraction = department_text_for_corr.find('Division of')
                    organizastion_text=""
                    if index_correspondence_department_extraction != -1:
                        department_correspondence = department_text_for_corr[index_correspondence_department_extraction:].strip().split(",")[0]
                        organizastion_text = department_text_for_corr[
                                                   index_correspondence_department_extraction+len(department_correspondence)+1:].strip()
                        print("department of correspondence", department_correspondence)

                    if index_correspondence_division_extraction != -1:
                        department_correspondence = department_text_for_corr[
                                                   index_correspondence_division_extraction:].strip().split(",")[0]
                        organizastion_text = department_text_for_corr[
                                                   index_correspondence_division_extraction+len(department_correspondence)+1:].strip()
                        print("department of correspondence", department_correspondence)

                    organization_list_for_corr = organizastion_text.split(',')

                    for org in organization_list_for_corr:
                        if "university" not in org and org != city_ and org != found_countries:
                            organization = org
                            print("9 is", organization)
                            if organization:
                                organization_name_correspondence = organization
                                break
            ...
            for line in lines:
                if 'Fax' in line or '+' in line:
                    ...
                    if numbers_with_plus:
                        ...
                        # Extract text that is not ORG or GPE and consider it as part of the address
            for token in doc:
                print("address for correspondence token check",token.text)
            addresses = [token.text for token in doc if (
                    token.ent_type_ not in {'ORG', 'GPE', 'PERSON'}
                    and token.text.lower() not in {'correspondence', 'text', 'contact', 'author',
                                                   'email', '*', 'Fax', 'Tel'}
                    and token.text not in email
                    and token.text.strip()
                    and
                    token.text != department_correspondence
                    and token.text != fax_number_correspondence
                    and token.text != phone_number_correspondence
            )]
            ...
            for string in pin_codes:
                ...
                if digit_count > max_digit_count:
                    max_digit_count = digit_count
                    postal_code_corr = string
            if not postal_code_corr in fax_number_correspondence or not postal_code_corr in phone_number_correspondence:
                postal_code_correspondence = postal_code_corr

            # Alternate number
            alternate_number = ""
            # print("Correspondence Author Alternate number:", alternate_number)

        if all(char in ",." for char in address_1):
            # If it contains only commas and a full stop, delete it
            address_1 = ""
        if all(char in ",." for char in address_1_correspondence):
            # If it contains only commas and a full stop, delete it
            address_1_correspondence = ""
